...
CMuroaTreeModel::CMuroaTreeModel( )  {
...
QVariant CMuroaTreeModel::data(const QModelIndex &index, int role) const {
...
Qt::ItemFlags CMuroaTreeModel::flags(const QModelIndex &index) const {
	return QAbstractItemModel::flags(index);
}

QVariant CMuroaTreeModel::headerData(int section, Qt::Orientation orientation, int role) const {
	if(role != Qt::DisplayRole)
	{
		return QVariant();
	}
...
QModelIndex CMuroaTreeModel::index(int row, int column, const QModelIndex &parent) const {
...
QModelIndex CMuroaTreeModel::parent(const QModelIndex &index) const {
	CItemBase *item = itemFromIndex(index);
...
bool CMuroaTreeModel::hasChildren( const QModelIndex & parent ) {
	CItemBase *parentItem = itemFromIndex(parent);
...
int CMuroaTreeModel::rowCount(const QModelIndex &parent) const {
	CItemBase *parentItem = itemFromIndex(parent);
	if (!parentItem)
		return 0;
...
int CMuroaTreeModel::columnCount(const QModelIndex &parent) const {
	return 6;
}

bool CMuroaTreeModel::beginInsertItems( const int pos, const int count, const CCategoryItem* parent ) {
	QModelIndex parentIndex = index(parent);
    QAbstractItemModel::beginInsertRows(parentIndex, pos, pos + count - 1);
    return true;
}

bool CMuroaTreeModel::endInsertItems( ) {
	QAbstractItemModel::endInsertRows();
	return true;
}

bool CMuroaTreeModel::beginRemoveItems( const int pos, const int count, const CCategoryItem* parent ) {
	QModelIndex parentIndex = index(parent);
	QAbstractItemModel::beginRemoveRows(parentIndex, pos, pos + count - 1);
	return true;
}

bool CMuroaTreeModel::endRemoveItems( ) {
    QAbstractItemModel::endRemoveRows();
	return true;
}


CItemBase* CMuroaTreeModel::itemFromIndex(const QModelIndex & index) const
{
	if (index.isValid()) {
...
QModelIndex CMuroaTreeModel::index(const CItemBase* item, int column) const {
...
QModelIndex CMuroaTreeModel::indexFromItem(const CItemBase* item) const {
	CCategoryItem* parent = item->getParent();
...
QVariant CMuroaTreeModel::dataFromColumn(CItemBase* item, int column) const {
