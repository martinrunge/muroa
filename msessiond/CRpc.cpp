...
void CRpc::onGetSessionState(uint32_t jobID, unsigned  knownRev) {
	m_connection->sendLatestSessionStateRev(jobID, knownRev);
}
...
void CRpc::onEditSessionState(uint32_t jobID, unsigned  fromRev, unsigned toRev, std::string sessionStateDiff) {
	CmdEditSessionState* ess = new CmdEditSessionState(fromRev, toRev, sessionStateDiff);
	m_connection->incomingCmd(ess);
}
...
void CRpc::sendCmd(muroa::Cmd* cmd) {
	...
	case Cmd::EDIT_SESSION_STATE:
	{
		...
		editSessionState(cmd_ess->id(), cmd_ess->getFromRev(), cmd_ess->getToRev(), cmd_ess->getDiff());
		break;
	}
	default:
		break;
